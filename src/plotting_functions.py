...
def plot_log_operations_bar_chart(
    ...
    df = df[~((df["tensor_network"] == "BB MSP") & (df["num_qubits"] == 30) & (df["methods"] == "['kahypar']"))]  # remove BB MSP with 30 qubits, don't have full data

    if method is not None:
        df = df[df["methods"].apply(lambda x: f'{method}' in x)]

    if os.path.exists("results/optimal_costs.csv"):
        optimal_df = pd.read_csv("results/optimal_costs.csv", sep=";")
    ...
    for idx, method in enumerate(methods):
        ...
        grouped_df_not_log = (
            df_m.groupby(["tensor_network", "num_qubits", "cost_fn"])["operations_with_bruteforce"]
            .agg(
                avg_ops="mean",
                std_dev="std",
                min_ops="min",
        ...
        not_log_df["improvement_default_custom"] = round(not_log_df["flops"] / not_log_df["custom_flops"], 2)
        ...
        final_df["brute_force_log"] = np.log2(final_df["brute_force_cost"].astype(float))
        ...
        merged = merged.sort_values(["tensor_network", "num_qubits"], ascending=[True, True])
        ...
        for i, row in merged.iterrows():
            bar_height = max(
                row["flops"] + row["default_std"], row["custom_flops"] + row["custom_std"]
            )
        ...
        ax.set_xticklabels(
            [f"n={nq}" for rep, nq in zip(merged["tensor_network"], merged["num_qubits"])], fontsize=11
        )
        ...
        for group_name, indices in groups.items():
            ...
            ax.text(x_center, ymin - 16, short_label, ha="center", va="top", fontsize=13, rotation=60,      # slanted
...
def plot_time_distributions_from_df(data, bins=30, out_file="time_distributions.png", method=None,):
    """
    ...
    for i, network in enumerate(REP_ORDER):
        ...
        if not custom_flops_times.empty:
            ...
            ax.bar(bin_edges[:-1], counts / counts.sum(), width=np.diff(bin_edges),
            alpha=1, color=GROUP_COLORS[network], label="SST")

        if not flops_times.empty:
            counts, bin_edges = np.histogram(flops_times, bins=bins)
            ax.bar(bin_edges[:-1], counts / counts.sum(), width=np.diff(bin_edges),
                alpha=0.6, color="gray", label="Dense")
...
plot_time_distributions_from_df("results/64_trials_results.csv", out_file="time_comparison_k.png", method="kahypar")

plot_log_operations_bar_chart(
    "results/64_trials_results.csv",
    "bar_chart_test.png",
)

plot_tensor_sparsity_distribution("results/tensor_sparsity_info.csv", "tensor_sparsity_dist_small.png")


plot_operations_comparison_scatter("results/wep_calculations_operations_comparison.csv", "scatter_plot_comparison.png")
