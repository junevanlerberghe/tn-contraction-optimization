...
df_custom = pd.read_csv("outputs/combined_data/custom_subrank_cost_results.csv", sep=";")
...
def prepare_data(df_default, df_custom, distance):
    ...
    custom_stats = (
        df_custom_filtered.groupby("representation")["total_ops"]
...
final_dfs = {
    ...
    "d4": prepare_data(df_default, df_custom, 4),
...
df = pd.read_csv("new_length_estimate_calcs/submatrix_rank.csv", sep=";")
...
# Plotting function
def plot_by_rep(x_col, ax, title):
    for rep in representations:
        subdf = df[df["representation"] == rep]
        ax.scatter(subdf[x_col], subdf["total_ops_count"], label=rep, color=color_map[rep])
    ax.set_xlabel(x_col)
    ax.set_xscale("log")
    ax.set_yscale("log")
...
plot_by_rep("score cotengra", axs[1], "Operations vs Cotengra Score")
plot_by_rep("pcm cost", axs[2], "Operations vs PCM Cost Model")
plot_by_rep("new legs cost", axs[3], "Operations vs (m + n + min(m, n))")
#plot_by_rep("avg_legs", axs[4], "Operations vs Average Legs Cost")
plot_by_rep("top_5_legs", axs[4], "Operations vs Top 5 Legs Cost")
plot_by_rep("total_submatrix_rank_cost", axs[5], "Operations vs Open Cols Rank Cost")
#plot_by_rep("top_2_legs", axs[6], "Operations vs Top 2 Legs Cost")
...
fig.suptitle("Cost vs Scores for d = 4", fontsize=16)
plt.tight_layout()
plt.savefig("new_length_estimate_calcs/submatrix_results.png")
plt.close()'''
...
plt.savefig("self_trace_scatter.png")
plt.close()