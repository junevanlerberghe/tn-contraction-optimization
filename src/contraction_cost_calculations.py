...
def generate_rotated_surface_code(d):
    ...
    # 2. Iterate over potential stabilizer centers
    # We iterate coordinates (r, c) representing the top-left corner of a check.
    # The range [-1, d] covers all necessary boundary checks.
    # Iterating 'c' then 'r' matches the Z-check ordering in your example best.
    for c in range(-1, d):
        for r in range(-1, d):
            
            # Determine Check Type based on lattice parity
            # Even sum -> X check, Odd sum -> Z check
            is_x_check = ((r + c) % 2 == 0)
            
            # Identify the 4 surrounding qubits for this check center
            ...
            # 3. Apply Boundary Rules to keep/discard checks
            # Weight 4 checks are always kept (Bulk).
    ...
    if stabilizers_x and stabilizers_z:
        ...
        H = np.array([]) # Handle degenerate cases if necessary
